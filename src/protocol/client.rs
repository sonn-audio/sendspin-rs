...
// The binary wire format lives in `sendspin-proto`: it has no direction, and the server needs
// to write exactly what this reads. Re-exported rather than moved out of sight, so every path
// that named these types through this module still resolves.
pub use sendspin_proto::binary::{
    binary_types, pack, pack_player_audio, pack_source_audio, ArtworkChunk, AudioChunk,
    BinaryFrame, VisualizerChunk, HEADER_LEN,
};
